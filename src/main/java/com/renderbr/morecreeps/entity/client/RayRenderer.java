...
public class RayRenderer extends EntityRenderer<RayEntity> {
    private RayModel model;

    public RayRenderer(EntityRendererProvider.Context context) {
        super(context);
        this.model = new RayModel(context.bakeLayer(RayModel.LAYER_LOCATION));
    }

    public void render(RayEntity pEntity, float pEntityYaw, float pPartialTicks, PoseStack pMatrixStack, MultiBufferSource pBuffer, int pPackedLight) {
    ...
    public ResourceLocation getTextureLocation(RayEntity pEntity) {
        return new ResourceLocation(MoreCreeps.MODID, "textures/entity/ray.png");
    }
